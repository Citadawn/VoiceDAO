...
public class InfoIconHelper {
     ...
     */
    public static void setupInfoIcon(Context context, ImageView infoIcon, 
                                   int titleResId, int contentResId) {
        if (infoIcon != null && context != null) {
            infoIcon.setOnClickListener(v -> {
                showInfoDialog(context, titleResId, contentResId);
            });
        }
    }
    ...
    public static void showInfoDialog(Context context, int titleResId, int contentResId) {
        if (context != null) {
            new AlertDialog.Builder(context)
                .setTitle(titleResId)
                .setMessage(HtmlCompat.fromHtml(context.getString(contentResId), 
    ...
    public static void showInfoDialog(Context context, String title, String content) {
        if (context != null) {
            new AlertDialog.Builder(context)
                .setTitle(title)
                .setMessage(HtmlCompat.fromHtml(content, 
                                               HtmlCompat.FROM_HTML_MODE_LEGACY))
                .setPositiveButton(R.string.tts_support_info_button, null)
                .show();
        }
     ...
     * @return 是否已设置点击监听器
     */
    public static boolean isInfoIconSetup(ImageView infoIcon) {
        return infoIcon != null && infoIcon.hasOnClickListeners();
    }
} 