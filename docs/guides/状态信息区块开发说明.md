...
- **国际化**：所有文本内容必须引用 `@string/*` 资源，禁止硬编码（字符串资源无论使用频率如何都必须使用资源）
- **颜色规范**：文本颜色优先使用主题属性 `?attr/colorOnSurface`（支持深色/浅色模式）
- **字号规范**：字号优先从 `@dimen` 获取，高频使用（3次以上）必须使用资源，低频使用（1-2次）可以硬编码
- **语义色使用**：若需状态语义色（如警示/支持度），使用项目语义色资源（如 `@color/accent_warning`、
  `@color/tts_support_*`）
...
- **分层与分区**：如需在代码中获取或更新状态，请在 `MainActivity` 中的 `// region 状态信息区块 // endregion`
  分区内维护相关逻辑
- **普通与调试分区**：常规项置于区块上部；调试相关项置于 `layoutStatusDebugSection` 内（浅橙分隔线居中嵌「调试信息」小标题，`sp_15`、`debug_mode_accent` 加粗，介于 `sp_16` 区块标题与 `sp_13` 项标签之间）。至少一项调试状态可见时由 `updateStatusDebugSectionVisibility()` 显示该分区
...
- **主题属性**：文本颜色优先使用 `?attr/colorOnSurface`，支持深色/浅色模式
