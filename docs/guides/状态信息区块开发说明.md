...
- **颜色与尺寸**：见 [开发规范](./开发规范.md)（2 次及以上用 `@color/*` / `@dimen/*`；`colors.xml` 中相同色值只保留一个资源名）
...
- **分层与分区**：如需在代码中获取或更新状态，请在 `MainActivity` 中的 `// region 状态信息区块 // endregion`
  分区内维护相关逻辑
- **普通与调试分区**：常规项置于区块上部；调试相关项置于 `layoutStatusDebugSection` 内（浅橙分隔线居中嵌「调试信息」小标题，`sp_15`、`accent_warning` 加粗，介于 `sp_16` 区块标题与 `sp_13` 项标签之间）。至少一项调试状态可见时由 `updateStatusDebugSectionVisibility()` 显示该分区
...
private void updateNetworkStatus() {
    ...
    if (isConnected) {
        ...
        tvNetworkStatus.setTextColor(ContextCompat.getColor(this, R.color.tts_support_full));
    } else {
...
private void updateAudioSaveStatus() {
    ...
    switch (status) {
        case READY:
            ...
            tvAudioSaveStatus.setTextColor(ContextCompat.getColor(this, R.color.tts_support_full));
            break;
        case SAVING:
