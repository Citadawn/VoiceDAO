...
语道 (VoiceDAO) 是一个专注于文本转语音（TTS）功能的 Android 应用。用户可输入任意文本，调节语速和音高，一键朗读。该项目依赖于系统当前设置的
...
- **调试模式**：开发者诊断能力；在主界面 Toolbar **连点「语道」7 次**进入（第 4 次起 Toast 提示剩余次数），勾选调试项后启用；⋮ 菜单 **退出调试模式** 或冷启动可退出。详见 [调试模式指南](./docs/guides/调试模式添加调试项指南.md)
...
- **全架构支持**：一个 APK 即可在所有 Android 设备上运行，支持 ARM（32/64 位）、x86（32/64 位）等所有 CPU 架构，无需为不同架构单独下载
...
语道通过系统 TTS 引擎朗读文本，应用本身不内置语音包。首次使用或更换引擎时，建议先配置系统默认引擎，再在主界面输入文本并朗读。
...
|------|------|
| 1 | 打开语道，进入主界面。 |
| 2 | 点击右上角 **⋮** 溢出菜单，选择 **TTS设置**。 |
| 3 | 在系统打开的「文字转语音」设置页中，将 **首选引擎** 设为需要的 TTS 引擎（不同机型文案可能略有差异，例如「选择 TTS 引擎」）。 |
| 4 | 返回语道主界面。若刚切换引擎，留意状态信息中的 **TTS引擎状态** 变为就绪；切换后应用会重新初始化引擎。 |
| 5 | 在文本输入框输入要朗读的内容（可点击输入框进入大文本编辑器整理长文）。 |
| 6 | 按需设置 **语速**、**音高**、**语言**、**发音人**（须先选语言再选发音人；各项旁的信息图标可查看说明）。 |
| 7 | 点击 **朗读** 开始播放。 |

```
打开语道 → ⋮ 菜单 → TTS设置 → 系统页选择首选引擎 → 返回主界面
→ 输入文本 → 设置语速 / 音高 / 语言 / 发音人 → 朗读
```

**保存为音频（可选）：** 完成引擎配置并输入文本后，在状态信息区块 **设置保存目录**，再点 **保存为音频** 并输入文件名；合成进度见状态区，可 **取消保存**。

另：系统 TTS 设置也可从手机「设置」搜索「文字转语音」进入（见下文）；⋮ 菜单中的 **TTS浏览器** 可查看已安装引擎及语言、发音人；部分引擎对应用内语速、音高的支持有限，见 [常见问题](./docs/faq/常见问题.md)。
...
- [开发规范](./docs/guides/开发规范.md) - 代码结构、资源与国际化约定

### 功能开发指南

- [Toast 开发指南](./docs/guides/Toast%20开发指南.md) - Toast 消息提示的使用规范和最佳实践
- [状态信息区块开发说明](./docs/guides/状态信息区块开发说明.md) - 主界面状态信息的开发规范
- [调试模式添加调试项指南](./docs/guides/调试模式添加调试项指南.md) - 为调试模式新增调试项的步骤
